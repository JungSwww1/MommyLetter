...
export const createDiary = async (data: FormData) => {
    await axios.post('/diary', data, config).then((response) => console.log(response.data)).catch((error) => console.log(error.response.data.message)) //메시지를 지정해 준 경우);
};
// 특정 사용자의 일기를 가져오는 함수
...
export const updateDiary = async (data: DiaryUpdateRequestProps) => {
    ...
    await axios.patch(`/diary/${data.diaryId}`, data).then((response) => console.log(response.data)).catch((error) => console.log(error.response.data.message)) //메시지를 지정해 준 경우);
